...
export function vendorDependencies(params: { distDirPath: string }) {
    const { distDirPath } = params;

    const vendorDirPath = pathJoin(distDirPath, "vendor");
    const cacheDirPath = pathJoin(cacheDirPath_base, "vendorDependencies");
